    ...
    <div class="row">
            ...
            if (!empty($this->data['License']['image'] || !empty($this->data['License']['Image']))) {
                ...
                if (count($gallery) > 1) {
                    foreach ($gallery AS $orig => $thumb) {
                        ?><a href="#" data-orig="<?php echo $orig; ?>" data-username="<?php echo $thumb['username']; ?>" data-userid="<?php echo $thumb['userid']; ?>" class="imgZoomSwitch"><img src="<?php echo $thumb['small']; ?>" class="img-thumbnail" style="width: 50px;" /></a><?php
                       }
                   }
                   ?>
                <p>&nbsp;</p>
        ...
        <div class="col-md-6">
            <dl class="dl-horizontal">
                ...
                <dt>註銷日期</dt>
                <dd><?php
                    if (!empty($this->data['License']['cancel_date'])) {
                    ...
                    if (!empty($this->data['License']['ingredient'])) {
                        ...
                        foreach ($majorIngredients AS $ingredient) {
                            ...
                            if (isset($ingredientKeys[$ingredient])) {
                                echo $this->Html->link(
                                        $this->Html->tag('label', $ingredient, array('class' => 'label label-default', 'style' => 'cursor: pointer')), '/ingredients/view/' . $ingredientKeys[$ingredient], array('class' => 'text-ellipsis', 'escape' => false)
                                ) . '<br>';
                            } else {
                                echo $this->Html->link(
                                        $this->Html->tag('label', $ingredient, array('class' => 'label label-default', 'style' => 'cursor: pointer')), '/drugs/index/' . $ingredient, array('class' => 'text-ellipsis', 'escape' => false)
                                ) . '<br>';
                            }
                            echo '</div>';
                        }
                ...
                <dd><?php
                    if (!empty($this->data['License']['Vendor']['name'])) {
                        echo $this->Html->link(
                                $this->Html->tag(
                                        'label', $this->data['License']['Vendor']['name'], array(
                                    'class' => 'label label-default',
                                    'style' => 'cursor: pointer')
                                ), '/vendors/view/' . $this->data['License']['Vendor']['id'], array('escape' => false)
                        );
                    } else {
                        echo '<span class="text-muted">無紀錄</span>';
                    }
                    ...
                    if (!empty($this->data['License']['Vendor']['tax_id'])) {
                        echo $this->Html->link(
                                $this->Html->tag('label', $this->data['License']['Vendor']['tax_id'], array(
                                    'class' => 'label label-default',
                                    'style' => 'cursor: pointer')
                                ), 'http://gcis.nat.g0v.tw/id/' . $this->data['License']['Vendor']['tax_id'], array('target' => '_blank', 'escape' => false)
                        );
                    } else {
                        echo '<span class="text-muted">無紀錄</span>';
                    }
...
echo $this->Html->link(
        $this->Html->tag('label', $this->data['Vendor']['name'], array(
            'class' => 'label label-default',
            'style' => 'cursor: pointer')
        ), '/vendors/view/' . $this->data['Vendor']['id'], array('class' => 'text-ellipsis', 'escape' => false)
);
echo '</div>';
?>
                    </div>
...
?>&nbsp;
                ...
                if (!empty($drugs)) {
                    ...
                    foreach ($drugs AS $drug) {
                        ...
                        echo $this->Html->link(
                                ...
                                ), '/drugs/view/' . $drug['Drug']['id'], array('class' => 'text-ellipsis', 'escape' => false)
                        );
                        echo '<br></div>';
                    }
                    echo '</dd>';
            ...
            <div class="col-md-12">
                ...
                <?php
                $noteCount = 0;
                $keysToCheck = array('info', 'notices', 'side_effects', 'interactions');
...
<?php if (!empty($ingredients)) { ?>
                    ...
                    <table class="table table-bordered table-condensed table-striped">
                        ...
                        <tbody><?php
                                ...
                                echo $this->Html->link(
                                        ...
                                        ), '/ingredients/view/' . $ingredient['IngredientsLicense']['ingredient_id'], array('escape' => false)
                                );
                                ?>
                                    </td>
            ...
            <div class="col-md-12">
                ...
                foreach ($this->data['License']['Category'] AS $category) {
                    echo $this->Html->link(
                            $category['code'], 'http://www.whocc.no/atc_ddd_index/?code=' . $category['code'] . '&showdescription=yes', array('target' => '_blank')
                    );
                    ...
                    foreach ($categoryNames[$category['CategoriesLicense']['category_id']] AS $item) {
                        ...
                        if ($loopCount >= $categoryNamesLen) {
                            echo $this->Html->tag(
                                    'li', $this->Html->link(
                                            $item['Category']['name'], '/drugs/category/' . $item['Category']['id'], array('style' => 'color: #16a085; cursor: pointer;')
                                    ), array('class' => 'active')
                            );
                        } else {
                            echo $this->Html->tag(
                                    'li', $this->Html->link(
                                            $item['Category']['name'], '/drugs/category/' . $item['Category']['id']
                                    )
                            );
                        }
                    }
